...
/// <summary>
/// An implementation of RFC 6079, It can be used to access some functionalities on the InternetGatewayDevice on your network without having to know anything about the required input/output parameters. All device functions are not provided.
/// </summary>
internal sealed class GatewayDevice
{
    ...
    private GatewayDevice(IPAddress ip, string data)
    {
        InternalClient = ip;
    ...
    public static bool TryNew(IPAddress ip, out GatewayDevice gateway)
    {
        ...
        foreach (string type in searchMessageTypes)
        {
            ...
            try
            {
                ...
                gateway = new GatewayDevice(ip, Encoding.ASCII.GetString(buffer, 0, recievedCount));
                return true;
            }
            catch
            {
    ...
    private static (string serviceType, string controlURL) GetInfo(string location)
    {
        ...
        foreach (XElement service in services)
        {
            ...
            foreach (var node in service.Nodes())
            {
                ...
                {
                    case "servicetype":
                        serviceType = n.Value.Trim();
                        break;
                    case "controlurl":
                        controlURL = n.Value.Trim();
                        break;
                }
            }
